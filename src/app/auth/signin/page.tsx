      ...
      <div className="w-full max-w-md bg-white border border-gray-100 rounded-2xl p-6 space-y-4">
        ...
        <div className="flex gap-2 text-sm">
          {(['signin', 'register', 'kid'] as const).map((m) => (
            <button key={m} onClick={() => { setMode(m); setError(null); }}
              className={`flex-1 py-2 rounded-lg font-medium ${mode === m ? 'bg-yellow-400 text-black' : 'bg-gray-100 text-gray-600'}`}>
        ...
        {mode !== 'kid' && (
          <div className="space-y-3">
            ...
            <input className="w-full border rounded-lg px-3 py-2 text-sm" type="password" placeholder="Password (8+)"
            ...
            <button disabled={busy} onClick={() => signIn('google', { callbackUrl: '/' })}
              className="w-full py-2.5 border border-gray-200 rounded-lg text-sm font-medium hover:bg-gray-50">
        ...
        {mode === 'kid' && (
          <div className="space-y-3">
            ...
            <hr />
            <input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="word.word"
        ...
        <Link href="/" className="block text-center text-sm text-gray-500 hover:text-gray-800">← Back</Link>
      </div>
    </main>
  );
}
